...
import com.ssafy.questory.auth.config.jwt.LoginUserDetailsService;
...
class MemberServiceTest {
    ...
    @Test
    @DisplayName("회원가입 성공: 닉네임 중복이어도 허용 (email만 유니크)")
    void register_success_evenIfNicknameDuplicated() throws Exception {
        RegisterRequestDto req = new RegisterRequestDto(
                "another@example.com",
                "Aa1!aaaa",
                "Aa1!aaaa",
        ...
        given(memberRepository.findByEmail("another@example.com")).willReturn(Optional.empty());
        given(passwordEncoder.encode("Aa1!aaaa")).willReturn("ENC");
        ...
        assertThat(res.memberId()).isEqualTo(2L);
        assertThat(res.email()).isEqualTo("another@example.com");
        assertThat(res.nickname()).isEqualTo("테스터");

        then(memberRepository).should(times(1)).register(any(Member.class));
        then(memberPasswordRepository).should(times(1)).register(2L, "ENC");
    }
